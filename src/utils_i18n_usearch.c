...
int i18n_usearch_get_matched_text(const i18n_usearch_h strsrch, i18n_uchar *result_w,
                                  int32_t result_capacity, int32_t *len_matched_text)
{
    UErrorCode err = U_ZERO_ERROR;
                        ...
                        i18n_usearch_h *search_iter)
{
    ...
    *search_iter =
        usearch_open(pattern, pattern_len, text, text_len, locale, (UBreakIterator *) break_iter,
                     (UErrorCode *) & err);
    int result = _i18n_error_mapping(err);
...
int i18n_usearch_create_new(const i18n_uchar *pattern, int32_t pattern_len,
                            const i18n_uchar *text, int32_t text_len, const char *locale,
                            i18n_ubreak_iterator_h break_iter, i18n_usearch_h *search_iter)
{
    retv_if(search_iter == NULL, I18N_ERROR_INVALID_PARAMETER);
    UErrorCode icu_error = U_ZERO_ERROR;
    *search_iter =
        (i18n_usearch_h) usearch_open(pattern, pattern_len, text, text_len, locale,
                                      (UBreakIterator *) break_iter, &icu_error);
