...
int i18n_udate_create(i18n_udate_format_style_e time_style, i18n_udate_format_style_e date_style,
					  const char *locale, const i18n_uchar *tz_id, int32_t tz_id_len,
					  const i18n_uchar *pattern, int32_t pattern_len, i18n_udate_format_h *format)
{
	retv_if(format == NULL, I18N_ERROR_INVALID_PARAMETER);

	UErrorCode icu_error = U_ZERO_ERROR;
	*format =
		udat_open(time_style, date_style, locale, tz_id, tz_id_len, pattern, pattern_len,
				  &icu_error);
...
int i18n_udate_format_date(const i18n_udate_format_h format, i18n_udate date_to_format,
						   i18n_uchar *result, int32_t result_len, i18n_ufield_position_h pos,
						   int32_t *buf_size_needed)
{
	...
	UErrorCode icu_error = U_ZERO_ERROR;
	*buf_size_needed =
		udat_format(format, date_to_format, result, result_len, (UFieldPosition *) pos, &icu_error);
...
int i18n_udate_parse(const i18n_udate_format_h format, const i18n_uchar *text, int32_t text_length,
					 int32_t *parse_pos, i18n_udate *parsed_date)
{
	...
	UErrorCode icu_error = U_ZERO_ERROR;
	*parsed_date =
		udat_parse((const UDateFormat *)format, text, text_length, parse_pos, &icu_error);
...
int i18n_udate_parse_calendar(const i18n_udate_format_h format, i18n_ucalendar_h *calendar,
							  const i18n_uchar *text, int32_t text_length, int32_t *parse_pos)
{
	if (format == NULL || calendar == NULL || text == NULL || text_length < -1)
		return I18N_ERROR_INVALID_PARAMETER;

	UErrorCode icu_error = U_ZERO_ERROR;
	udat_parseCalendar((const UDateFormat *)format, calendar, text, text_length, parse_pos,
					   &icu_error);
...
int32_t i18n_udate_to_pattern(const i18n_udate_format_h format, i18n_ubool localized,
							  i18n_uchar *result, int32_t result_length)
{
	...
	int32_t needed_buffer_size =
		udat_toPattern((const UDateFormat *)format, localized, result, result_length, &icu_error);
...
int i18n_udate_apply_pattern(i18n_udate_format_h format, i18n_ubool localized,
...
int32_t i18n_udate_get_symbols(const i18n_udate_format_h format,
							   i18n_udate_format_symbol_type_e type, int32_t symbol_index,
							   i18n_uchar *result, int32_t result_length)
{
	if (format == NULL || type < I18N_UDATE_FORMAT_SYMBOL_TYPE_ERAS
		|| type > I18N_UDATE_FORMAT_SYMBOL_TYPE_STANDALONE_SHORTER_WEEKDAYS || symbol_index < 0) {
		set_last_result(I18N_ERROR_INVALID_PARAMETER);
		return -1;
	}
	...
	UErrorCode icu_error = U_ZERO_ERROR;
	int32_t needed_buffer_size =
		udat_getSymbols((const UDateFormat *)format, type, symbol_index, result, result_length,
						&icu_error);
...
int i18n_udate_set_symbols(i18n_udate_format_h format, i18n_udate_format_symbol_type_e type,
						   int32_t symbol_index, i18n_uchar *value, int32_t value_length)
{
